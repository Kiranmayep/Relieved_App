...
public class HelperUtils {
    ...
    public static void applyColours(Activity activity, int colourPrimary, boolean colourNavbar) {
        ...
        window.setStatusBarColor(com.example.relieved.HelperUtils.darkenColor(colourPrimary, 0.2));
    ...
    public static ArrayList<File> getFiles(Context context) {
        File[] files = context.getFilesDir().listFiles(new FilenameFilter() {
            ...
            public boolean accept(File dir, String name) {
                return name.toLowerCase().endsWith(com.example.relieved.HelperUtils.TEXT_FILE_EXTENSION);
            }
        });
    ...
    public static boolean fileExists(Context context, String fileName) {
        File file = context.getFileStreamPath(fileName + com.example.relieved.HelperUtils.TEXT_FILE_EXTENSION);
        return file.exists();
    }

    public static void writeFile(Context context, String fileName, String fileContent) {
        try {
            OutputStreamWriter out = new OutputStreamWriter(context.openFileOutput(fileName + com.example.relieved.HelperUtils.TEXT_FILE_EXTENSION, 0));
    ...
    public static String readFile(Context context, String fileName) {
        ...
        if (fileExists(context, fileName)) {
            try {
                InputStream in = context.openFileInput(fileName + com.example.relieved.HelperUtils.TEXT_FILE_EXTENSION);
                if (in != null) {
